...
export default function WorkoutReview({ navigation, route }) {
  ...
  const [myStatsLoading, setMyStatsLoading] = React.useState(false);
  ...
  useEffect(() => {
    createListOfComparisons();
  }, [myStatsLoading, friendsStats]);

  async function loadAllMyStats() {
    // loading
  ...
  async function loadMyFriendsStats() {
  ...
  async function createListOfComparisons() {
    ...
    for (let i = 0; i < latestStats.exerciseInputData.length; ++i) {
      ...
      comparisonObject.averageReps = averageStats.exerciseInputData[i].reps
        ...
        : "";
      comparisonObject.averageWeight = averageStats.exerciseInputData[i].weight
  ...
  if (myStatsLoading || !friendsStats || !comparisonsList) {
    return (
